...
In the notation above, given an adjoint triangle for a functor $J$, there is a natural transformation $JR\xrightarrow{\Sigma} U$ which makes the following diagram commute:
	% https://q.uiver.app/#q=WzAsNCxbMiwwLCJVTEoiXSxbMCwwLCJKUkxKIl0sWzAsMSwiSlJFIl0sWzIsMSwiSiJdLFsxLDAsIlxcU2lnbWEgTEoiXSxbMiwxLCJKUjEiXSxbMywyLCJKXFxyaG8iXSxbMywwLCJcXGV0YSBKIiwyXV0=
	\[\begin{tikzcd}
		JRLJ && ULJ \\
		JRE && J
		\arrow["{\Sigma LJ}", from=1-1, to=1-3]
		\arrow["JR1", from=2-1, to=1-1]
...
\begin{proof}
	Define $ \Sigma $ as the composite:
		% https://q.uiver.app/#q=WzAsNCxbMiwxLCJVIl0sWzAsMSwiSlIiXSxbMiwwLCJVRVIiXSxbMCwwLCJVTEpSIl0sWzIsMCwiVVxcbXUiXSxbMywyLCJVMVIiXSxbMSwzLCJcXGV0YSBKUiJdLFsxLDAsIlxcU2lnbWEiLDJdXQ==
		\[\begin{tikzcd}
			ULJR && UER \\
			JR && U
			\arrow["U1R", from=1-1, to=1-3]
...
\begin{thm}[Adjoint Triangle Theorem]\cite{DubuAT}\label{thm:dubuc-coadjoint-triangle}
	...
	Assume that	$ \eta $ is the equaliser of the diagram
			...
			\end{tikzcd}\].
	Then, if the equaliser of the diagram
			% https://q.uiver.app/#q=WzAsNCxbMCwwLCJSTCJdLFsxLDEsIlJFUkwiXSxbMywwLCJSTFVMIl0sWzIsMSwiUkxKUkwiXSxbMCwxLCJcXHJobyBSTCIsMl0sWzAsMiwiUkxcXGV0YSJdLFsxLDMsIlIxUkwiLDJdLFszLDIsIlJMXFxTaWdtYSBMIiwyXV0=
			\[\begin{tikzcd}
				RL &&& RLUL \\
				& RERL & RLJRL
				\arrow["{RL\eta}", from=1-1, to=1-4]
				\arrow["{\rho RL}"', from=1-1, to=2-2]
				\arrow["R1RL"', from=2-2, to=2-3]
				\arrow["{RL\Sigma L}"', from=2-3, to=1-4]
			\end{tikzcd}\]
	exists in the category $ \mathbb{A}^{ \mathbb{B}} $, $ J  $ has a right adjoint, $ G $, and $ G $ is computed as this equaliser.  
\end{thm}
