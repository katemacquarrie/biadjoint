...
\begin{proof}
	...
	The identities are recalled:
		\begin{equation}
			...
			\begin{tikzcd}\label{thm:triangle-identities-adjunction}
				F & FUF & UFU & U \\
				& F & U
...
\begin{thm}
	Every monad $  T= (T,\eta, \mu) $ on $ \mathbb{X} $ determines an adjunction $ (F^T, U^T, \eta^T, \epsilon^T): \mathbb{X} \longrightarrow \mathbb{X}^T $
\end{thm}




To each monad $T$ we associate structures call $T$-algebras defined as follows.


In this section, we are interested in a relationship between monads and adjunctions: that being every monad produces an adjunction and \textit{vice versa}.


\begin{thm}
	Every monad $T=(T,\eta, \mu)$ on a category $ \mathbb{X}$ determines an adjunction $(F^T,U^T, \eta^T, \epsilon ^T): \mathbb{X} \longrightarrow \mathbb{ \mathbb{X}^{T}}$ from $ \mathbb{X}$ to the category of its $T$-algebras, $ \mathbb{X}^{T}$.
...
\begin{proof}
	We define
	\begin{enumerate}
		\item the right adjoint $U^{T}: \mathbb{X}^{T} \longrightarrow \mathbb{X}$ to be the forgetful functor, defined by $ U^{T}(X,\xi) = X$;
		\item the left adjoint $F^{T}: \mathbb{X} \longrightarrow \mathbb{X}^{T}$ to be the functor defined by $F^T(X)= (T(X), \mu_{X})$;
		\item the unit $\eta^{T}: 1_{ \mathbb{X}} \longrightarrow U^{T}F^{T}$ to be the same $\eta$ of the monad;
		\item the counit $ \epsilon^{ T}: F^{T}U^{T} \longrightarrow 1_{ \mathbb{X}^{T}} $ to be defined by $ \epsilon^{ T}_{(T(X),\mu _X)} = \mu_X$. 
...
\begin{thm}\label{thm:kiesli}
	Let $ (F,U,\eta,\epsilon):\mathbb{X}\longrightarrow \mathbb{A}$ be an adjunction
...
\begin{proof}
	We define $K: \mathbb{A} \longrightarrow \mathbb{X}^{T} $ by $K(A) = (U(A), U( \epsilon _A))$, and on morphisms $K(f)=U(f)$.
\end{proof}

There is one last detail that will make our diagram complete: the functor $K$ as defined above has a left adjoint, the functor $L: \mathbb{X}^{T} \longrightarrow \mathbb{A}$. 

\begin{thm}
	There is an adjunction $(L,K,\overline{\eta}, \overline{ \epsilon}): \mathbb{X}^{T} \longrightarrow \mathbb{A}$.
\end{thm}

\begin{proof}
	We define
	\begin{enumerate}
		\item the left adjoint $L: \mathbb{X}^{T} \longrightarrow \mathbb{A}$, where $L(X,\xi)$ is defined as the coequaliser of
		\[\begin{tikzcd}
			{FUF(X)} & {F(X)} & {L(X,\xi)}
		...
		\end{tikzcd}\]
		\item the right adjoint $K: \mathbb{A} \longrightarrow  \mathbb{X}^{T}$, defined by $K(A)=(U(A), U( \epsilon_A))$ on objects and $K(f)=U(f)$ on morphisms;
		\item the unit $\overline{\eta}: 1_{ \mathbb{X}^{T}} \longrightarrow  KL$ defined by $\overline{\eta}_{(X,\xi)}=U(\pi_{(X,\xi)})\eta_X$;
		\item the counit $\overline{ \epsilon}: LK \longrightarrow 1_{ \mathbb{A}}$ is defined for each $ \overline{ \epsilon}_A$ as the unique morphism making the diagram
		\[\begin{tikzcd}
			{FUFU(A)} & {FU(A)} & {L(U(A), U(\epsilon_A))}\\ 
			& & {A}
			\arrow["{\epsilon_{FU(A)}}", shift left, from=1-1, to=1-2]
			\arrow["{FU( \epsilon_A)}"', shift right, from=1-1, to=1-2]
			\arrow["{\pi_{(U(A), U( \epsilon_A))}}", from=1-2, to=1-3]
